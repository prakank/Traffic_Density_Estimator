...
int main(int argc, char* argv[]){
    ...
    // cout << argv[3] << typeid(argv[3]).name()<<"\n";
    if(argc==4){
        if(*argv[3]=="false"){
            Generate_Baseline=false;
            ifstream file("Source.csv");
            string out;
            int i=0;
            cout <<"Hey";
    ...
    if(Generate_Baseline)imshow("frame_temp", frame_temp);
    
    if (i < 4 && Generate_Baseline) setMouseCallback("frame_temp", onMouse, reinterpret_cast<void*>(&frame_temp));
    int j = 0;
    
    //Marking points as they are being selected
    while (i != 4 && Generate_Baseline) {
        if (j != i) {
    ...
    cout << argv[3] << " " << typeid(Generate_Baseline).name() << "\n";
    if(Generate_Baseline == true)cout <<"ehy\n";
    while(t++){

        if(Generate_Baseline == false)break;
        // cout << Generate_Baseline << "\n";
